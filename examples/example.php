<?php
/**
 * Alexa Scraper — Scrapeless LLM Chat Scraper (PHP example)
 *
...
echo PHP_EOL . "Answer:" . PHP_EOL . ($result["raw_text"] ?? $result["md_text"] ?? "") . PHP_EOL;

foreach ($result["references"] ?? [] as $ref) {
    echo "- " . ($ref["title"] ?? "") . " -> " . ($ref["url"] ?? "") . PHP_EOL;
}

foreach ($result["products"] ?? [] as $product) {
    echo "* " . ($product["title"] ?? "") . " (" . ($product["price"] ?? "") . ") -> " . ($product["url"] ?? "") . PHP_EOL;
}
